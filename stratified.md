...
my_theme <- theme_classic() +
 theme(
    ...
    axis.title = element_text(face = "italic", size = 10, color = "gray20"),
...
create_outcome_plot <- function(dtst, gv, gv_label_false, gv_label_true) {
  ...
  geom_text(aes(label = weight_string),
    ...
    fontface = "bold") +
  coord_flip() +
  scale_x_discrete(labels = c("not exposed \n to stressful life event",
    "exposed \n to stresful life event")) +
...
```R
create_outcome_plot(events_data) +
...
```R
create_outcome_plot_grid <- function(dtst, gv, gv_label_true = "TRUE", gv_label_false = "FALSE") {
  gv <- enquo(gv)
  outcome_plot <- create_outcome_plot(dtst, !!gv, gv_label_false, gv_label_true) +
...
```R
#plot_women
create_outcome_plot_grid(events_data, women,
  gv_label_false = "men",
  gv_label_true = "women") +
...
create_outcome_plot_grid(events_data, psychiatric_history,
...
```R
#plot_depression
create_outcome_plot_grid(events_data, depression,
...
plot_attempts <- create_outcome_plot_grid(events_data, attempts,
